...
- **HTTP server** — the SDK does not expose an HTTP server. Wire it into your own framework using the [transport recipes](/docs/client).
...
- **AWS permissions must be provisioned externally.** The SDK does not create IAM roles, S3 buckets, or Lambda functions. See [Provisioning](/docs/client/provisioning).
